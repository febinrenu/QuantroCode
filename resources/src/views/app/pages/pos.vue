  ...
  <div class="pos-codecanyon" style="display: flex; flex-direction: column; height: 100vh; background: #f7f7fb; color: #1f1f2c; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; -webkit-font-smoothing: antialiased; overflow: hidden;">
    ...
    <nav v-if="productsReady" class="pos-shell-header" style="display: flex; align-items: center; gap: 8px; padding: 0 16px; flex-shrink: 0; flex-grow: 0; border-bottom: 1px solid #e6e6ec; background: #ffffff;">
      ...
      <button
        ...
        class="pos-shell-register-status"
        type="button"
      ...
      >
        ...
        <span class="pos-register-copy">
          <span class="pos-register-title">{{ $t('Register') || 'Register' }}<span v-if="currentRegister && currentRegister.id"> {{ String(currentRegister.id).padStart(2, '0') }}</span><span v-else> 01</span></span>
          <span class="pos-register-state">{{ (currentRegister && currentRegister.status === 'open') || isOnline ? ($t('Online') || 'Online') : ($t('Closed') || 'Closed') }}</span>
        </span>
        <span
          v-if="isOnline"
          class="pos-register-legacy-state"
        ></span>
      </button>
      ...
      >
        <span class="pos-wh-trigger-icon">
          <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
            <path d="M12 3 4 7v5c0 4.5 3.2 7.9 8 9 4.8-1.1 8-4.5 8-9V7l-8-4Z"></path>
          </svg>
        </span>
        <span class="pos-wh-trigger-body">
          <span class="pos-wh-trigger-eyebrow">{{ $t('Warehouse') || 'Warehouse' }}</span>
          <span class="pos-wh-trigger-label">{{ posHeaderWarehouseLabel }}</span>
        </span>
      ...
      >
        ...
        <span class="pos-cat-trigger-body">
          <span class="pos-cat-trigger-eyebrow">{{ $t('Category') || 'Category' }}</span>
      ...
      >
        <span class="pos-cust-trigger-avatar">
          <span>{{ catInitial(posHeaderCustomerLabel) }}</span>
        </span>
        <span class="pos-cust-trigger-body">
          <span class="pos-cust-trigger-eyebrow">{{ $t('Customer') || 'Customer' }}</span>
          <span class="pos-cust-trigger-label">{{ posHeaderCustomerLabel }}</span>
        </span>
      ...
      <div class="pos-reference-actions">
        <button
          ...
          class="pos-reference-icon-btn">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
            <circle cx="9" cy="8" r="3.4"></circle>
            <path d="M2.8 20c.7-3.4 3.2-5.2 6.2-5.2s5.5 1.8 6.2 5.2"></path>
        ...
        <button
          ...
          class="pos-reference-icon-btn">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
            <path d="m19 5-14 14M6.5 9a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5ZM17.5 20a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Z"></path>
          </svg>
        </button>
        <button
          ...
          class="pos-reference-icon-btn">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
            <path d="M3 7V4h3M21 7V4h-3M3 17v3h3M21 17v3h-3M7 12h10"></path>
          </svg>
        </button>
        <button
          @click="print_last_receipt"
          :title="$t('print') || 'Print'"
          class="pos-reference-icon-btn">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
            <path d="M6 9V3h12v6M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"></path>
        ...
        <button
          ...
          class="pos-reference-icon-btn pos-reference-bell">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
            <path d="M18 9a6 6 0 1 0-12 0c0 6-2.5 7-2.5 7h17S18 15 18 9ZM10 20a2.2 2.2 0 0 0 4 0"></path>
          </svg>
          <span>3</span>
        </button>
    ...
    <div v-if="productsReady" class="pos-shell-main" style="flex: 1 1 auto; display: flex; flex-direction: row; align-items: stretch; min-height: 0; overflow: hidden;">
      ...
      <section class="pos-shell-section" style="flex: 1 1 auto; min-width: 0; display: grid; grid-template-rows: auto 1fr auto; min-height: 0; background: #ffffff; overflow: hidden;">
        ...
        <div
          v-if="pos_settings.show_categories && isOnline"
          class="pos-shell-category-strip"
        >
          <button
            type="button"
...
export default {
  ...
  computed: {
    ...
    posHeaderWarehouseLabel() {
      const label = this.selectedWarehouseLabel || '';
      if (!this.sale.warehouse_id || String(label).toLowerCase().includes('select')) {
        return this.$t('Main Warehouse') || 'Main Warehouse';
      }
      return label;
    },
    ...
    posHeaderCustomerLabel() {
      const label = this.selectedCustomerLabel || '';
      const normalized = String(label).toLowerCase();
      if (!this.selectedClientId || normalized.includes('walk') || normalized.includes('select')) {
        return this.$t('Walk-in Customer') || 'Walk-in Customer';
      }
      return label;
    },
...
@import url('https://fonts.googleapis.com/css2?family=Sora:wght@600;700;800&family=IBM+Plex+Sans:wght@400;500;600;700&family=IBM+Plex+Sans+Arabic:wght@400;500;600;700&family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500;600;700&display=swap');
