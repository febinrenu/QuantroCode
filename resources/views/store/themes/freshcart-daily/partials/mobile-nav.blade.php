@php $fcClient = Auth::guard('store')->user(); @endphp
<nav class="md:hidden fixed bottom-0 inset-x-0 z-30 bg-white/95 backdrop-blur border-t border-fc-green/15 pb-[env(safe-area-inset-bottom)]">
  <div class="grid grid-cols-4 h-16">
    <a href="{{ route('store.index') }}" class="flex flex-col items-center justify-center gap-0.5 {{ request()->routeIs('store.index') ? 'text-fc-green' : 'text-fc-inkSoft' }}">
    ...
    <a href="{{ route('store.shop') }}" class="flex flex-col items-center justify-center gap-0.5 {{ request()->routeIs('store.shop') ? 'text-fc-green' : 'text-fc-inkSoft' }}">
    ...
    <a href="{{ route('store.cart') }}" class="relative flex flex-col items-center justify-center gap-0.5 {{ request()->routeIs('store.cart') ? 'text-fc-green' : 'text-fc-inkSoft' }}">
      ...
      <span class="cart-count absolute top-1 right-6 min-w-[16px] h-4 px-1 rounded-full bg-fc-orange text-white text-[10px] font-bold inline-flex items-center justify-center">0</span>
      <span class="text-[10px] font-medium">{{ __('messages.Cart') }}</span>
    </a>
    @if($fcClient)
      <a href="{{ url('/online_store/account') }}" class="flex flex-col items-center justify-center gap-0.5 text-fc-inkSoft">
    ...
    @else
      <a href="{{ url('/online_store/login') }}" class="flex flex-col items-center justify-center gap-0.5 text-fc-inkSoft">
        <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 4-6 8-6s8 2 8 6"/></svg>
