    ...
    <div x-data="miniCart()">
      <template x-if="!items.length">
        <div class="text-center py-20 bg-white border border-fc-green/15 rounded-xl2">
          ...
          <a href="{{ route('store.shop') }}" class="mt-4 inline-flex h-11 px-6 bg-fc-green text-white font-bold text-xs rounded-full items-center">{{ __('messages.ContinueShopping') ?? 'Start Shopping' }}</a>
        </div>
      </template>
