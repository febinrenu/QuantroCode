@php $fxClient = Auth::guard('store')->user(); @endphp
<nav class="lg:hidden fixed bottom-0 inset-x-0 z-30 bg-[#0A0E1A]/95 backdrop-blur border-t border-fx-border pb-[env(safe-area-inset-bottom)]">
  <div class="grid grid-cols-5 h-16">
    <a href="{{ route('store.index') }}" class="flex flex-col items-center justify-center gap-0.5 {{ request()->routeIs('store.index') ? 'text-fx-cyan' : 'text-fx-mute' }}">
    ...
    <a href="{{ route('store.shop') }}" class="flex flex-col items-center justify-center gap-0.5 {{ request()->routeIs('store.shop') ? 'text-fx-cyan' : 'text-fx-mute' }}">
    ...
    <a href="{{ route('store.shop', ['sort' => 'price_asc']) }}" class="flex flex-col items-center justify-center gap-0.5 text-fx-mute">
    ...
    <a href="{{ route('store.cart') }}" class="relative flex flex-col items-center justify-center gap-0.5 {{ request()->routeIs('store.cart') ? 'text-fx-cyan' : 'text-fx-mute' }}">
      ...
      <span class="cart-count absolute top-1 right-6 min-w-[16px] h-4 px-1 rounded-full bg-fx-pink text-[#0A0E1A] text-[10px] font-bold inline-flex items-center justify-center">0</span>
      <span class="text-[10px] font-medium">{{ __('messages.Cart') }}</span>
    </a>
    @if($fxClient)
      <a href="{{ url('/online_store/account') }}" class="flex flex-col items-center justify-center gap-0.5 text-fx-mute">
    ...
    @else
      <a href="{{ url('/online_store/login') }}" class="flex flex-col items-center justify-center gap-0.5 text-fx-mute">
        <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 4-6 8-6s8 2 8 6"/></svg>
