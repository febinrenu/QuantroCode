    ...
    <div x-data="miniCart()">
      <template x-if="!items.length">
        <div class="text-center py-20 bg-white border border-urb-ink/15">
          ...
          <a href="{{ route('store.shop') }}" class="mt-4 inline-flex h-11 px-6 bg-urb-green text-white font-bold text-xs eyebrow items-center">{{ __('messages.ContinueShopping') ?? 'Start Shopping' }}</a>
        </div>
      </template>
