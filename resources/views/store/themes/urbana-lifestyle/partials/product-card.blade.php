...
<article class="product-card group bg-white flex flex-col">
  <a href="{{ $product['url'] }}" class="relative block aspect-square overflow-hidden bg-urb-cream" style="{{ !$product['image_url'] ? 'background:'.$product['placeholder_color'].'22' : '' }}">
    ...
    @elseif($product['stock_status'] === 'preorder')
      <span class="absolute top-2.5 left-2.5 bg-urb-orange text-white text-[10px] font-bold eyebrow px-2 py-1">{{ __('messages.PreOrder') ?? 'Pre-order' }}</span>
    @elseif($product['stock_status'] === 'out_of_stock')
      <span class="absolute top-2.5 left-2.5 bg-urb-ink/80 text-white text-[10px] font-bold eyebrow px-2 py-1">{{ __('messages.OutOfStock') ?? 'Out of stock' }}</span>
    @endif
  ...
  <div class="product-body pt-3 flex flex-col flex-1">
    ...
    @else
      <a href="{{ url('/online_store/login') }}" class="text-xs font-semibold text-urb-green underline mt-1">{{ __('messages.LoginToSeePrice') ?? 'Sign in to see price' }}</a>
    @endif
