...
<article class="product-card group bg-white border border-fx-ink/10 rounded-lg overflow-hidden flex flex-col hover:shadow-cardHover transition-shadow">
  <a href="{{ $product['url'] }}" class="relative block aspect-square overflow-hidden bg-fx-cream" style="{{ !$product['image_url'] ? 'background:'.$product['placeholder_color'].'22' : '' }}">
    ...
    @elseif($product['stock_status'] === 'preorder')
      <span class="absolute top-2.5 left-2.5 bg-fx-purple text-white text-[10px] font-bold px-2 py-1 rounded">{{ __('messages.PreOrder') ?? 'Pre-order' }}</span>
    @elseif($product['stock_status'] === 'out_of_stock')
      <span class="absolute top-2.5 left-2.5 bg-fx-ink/80 text-white text-[10px] font-bold px-2 py-1 rounded">{{ __('messages.OutOfStock') ?? 'Out of stock' }}</span>
    @endif
  </a>
  ...
  <div class="product-body p-3 flex flex-col flex-1">
    ...
    @else
      <a href="{{ url('/online_store/login') }}" class="text-xs font-semibold text-fx-purple underline mt-1">{{ __('messages.LoginToSeePrice') ?? 'Sign in to see price' }}</a>
    @endif
