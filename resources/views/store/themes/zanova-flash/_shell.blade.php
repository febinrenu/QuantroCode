@php
  $activeThemeSlug = 'zanova-flash';
  $themeTokens = \App\Support\StorefrontThemeRegistry::resolveTokens($activeThemeSlug, $s->theme_tokens ?? []);
  $accent500 = $themeTokens['color-accent-500'] ?? '#FF3B30';
  $accent600 = $themeTokens['color-accent-600'] ?? '#00F0FF';
  $accent700 = $themeTokens['color-accent-700'] ?? '#7C3AED';
  $accent800 = $themeTokens['color-accent-800'] ?? '#0B0B14';
...
<body class="bg-zanova-bg text-zanova-text min-h-screen flex flex-col antialiased selection:bg-zanova-yellow selection:text-zanova-navy"
    ...
    @include('store.themes.zanova-flash.partials.header')

    <!-- Mobile Navigation Drawer -->
    @include('store.themes.zanova-flash.partials.mobile-nav')
    ...
    <!-- Dark Navy Footer -->
    @include('store.themes.zanova-flash.partials.footer')
