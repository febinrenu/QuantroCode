@php
  ...
  $sku = $p->code ?: ('DIGI-' . str_pad($p->id, 5, '0', STR_PAD_LEFT));
@endphp

<article class="digi-card product-card">
  <div class="digi-card-top">
    <span class="digi-license-pill">
      <span class="digi-pulse-dot"></span> DIGITAL LICENSE
    </span>
    <span class="digi-sku font-mono">{{ $sku }}</span>
  </div>

  <div class="digi-media">
    <a href="#" class="digi-img-link js-quick-view"
       data-id="{{ $p->id }}"
       ...
       @click.prevent>
      <img src="{{ $imgUrl }}" alt="{{ $p->name }}" loading="lazy" class="digi-img">
    </a>

    @if($isPreorderActive)
      <span class="digi-badge-pre">{{ __('messages.PreOrder') }}</span>
    @elseif(!$isAvailable)
      <span class="digi-badge-out">{{ __('messages.OutOfStock') }}</span>
    @endif

    <button type="button" class="digi-preview-btn js-quick-view"
            title="Preview Details & License"
            data-id="{{ $p->id }}"
            data-slug="{{ $productSlug }}"
            data-name="{{ e($p->name) }}"
            data-price="{{ number_format($minPrice, 2, '.', '') }}"
  ...
  <div class="digi-body product-body">
    @if($p->category)
      <div class="digi-cat-name">{{ $p->category->name }}</div>
    @endif

    <h3 class="digi-title product-title" title="{{ $p->name }}">
      {{ $p->name }}
    </h3>

    @if($descShort)
      <p class="digi-desc">{{ \Illuminate\Support\Str::limit($descShort, 80) }}</p>
    @endif

    {{-- License Tiers Indicator --}}
    @if($variants->isNotEmpty())
      <div class="digi-tiers-bar">
        <span class="digi-tier-chip">{{ $variants->count() }} License Editions Available</span>
      </div>
    @endif

    <div class="digi-pricing-row">
      <div class="digi-price-block">
        @if(empty($hidePrices))
          <span class="digi-price-lbl">LICENSE PRICE</span>
          <div class="digi-price font-mono price">{{ $currency }}{{ number_format($minPrice, 2, '.', ',') }}</div>
        @else
          <a href="{{ route('store.login.show') }}" class="text-xs text-indigo-400 font-semibold underline">Sign in for Price</a>
        @endif
      </div>

      @if(empty($hidePrices))
        <button type="button"
                class="digi-btn-get js-add-to-cart"
          ...
          <span>{{ $isPreorderActive ? __('messages.PreOrder') : 'Get License' }}</span>
        </button>
      @endif
    </div>
    <div class="js-add-status digi-add-status"></div>
  </div>
</article>
