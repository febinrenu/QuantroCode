@php
  $activeThemeSlug = 'nexora-trending';
  $themeTokens = \App\Support\StorefrontThemeRegistry::resolveTokens($activeThemeSlug, $s->theme_tokens ?? []);
  $accent500 = $themeTokens['color-accent-500'] ?? '#2457D6';
  $accent600 = $themeTokens['color-accent-600'] ?? '#7C3AED';
  $accent700 = $themeTokens['color-accent-700'] ?? '#F97316';
  $accent800 = $themeTokens['color-accent-800'] ?? '#101A3A';
...
<body class="bg-nex-bg text-nex-text antialiased min-h-screen flex flex-col selection:bg-nex-blue selection:text-white"
    ...
    <!-- Header -->
    @include('store.themes.nexora-trending.partials.header')

    <!-- Mobile Navigation Drawer -->
    @include('store.themes.nexora-trending.partials.mobile-nav')
    ...
    <!-- Footer -->
    @include('store.themes.nexora-trending.partials.footer')
