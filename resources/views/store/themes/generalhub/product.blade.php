...
<head>
@include('store.themes.generalhub._shell', ['pageTitle' => ($product['name'] ?? 'Product') . ' — ' . ($s->store_name ?? 'GeneralHub')])
</head>
      ...
      <div class="mt-16">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
          @foreach($relatedVms->take(4) as $pItem)
            @include('store.themes.generalhub.partials.product-card', ['product' => $pItem])
          @endforeach
        </div>
      </div>
    @endif
