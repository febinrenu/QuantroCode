...
<article class="product-card group bg-white rounded-xl border border-slate-200 shadow-card hover:shadow-cardHover transition-shadow overflow-hidden flex flex-col">
  <a href="{{ $product['url'] }}" class="relative block aspect-square overflow-hidden" style="{{ !$product['image_url'] ? 'background:'.$product['placeholder_color'].'22' : '' }}">
    ...
    @if($product['is_on_sale'])
      <span class="absolute top-2 left-2 bg-brand-orange text-white text-[11px] font-bold px-2 py-0.5 rounded-full">-{{ $product['discount_percent'] }}%</span>
    @elseif($product['stock_status'] === 'preorder')
      <span class="absolute top-2 left-2 bg-brand-blue text-white text-[11px] font-bold px-2 py-0.5 rounded-full">Pre-order</span>
    @elseif($product['stock_status'] === 'out_of_stock')
      <span class="absolute top-2 left-2 bg-slate-700 text-white text-[11px] font-bold px-2 py-0.5 rounded-full">Out of stock</span>
    @endif
  </a>

  <div class="product-body p-3 flex flex-col flex-1">
    @if($product['category_name'])
      <span class="text-[11px] font-semibold text-brand-blue uppercase tracking-wide">{{ $product['category_name'] }}</span>
    @endif
    <a href="{{ $product['url'] }}" class="product-title text-sm font-semibold text-brand-navy mt-0.5 line-clamp-2 hover:text-brand-blue" title="{{ $product['name'] }}">
      {{ $product['name'] }}
    </a>
    ...
    <div class="mt-auto pt-2">
      @if(!$product['hide_prices'])
        ...
        @if(count($product['variants']) > 0)
          <a href="{{ $product['url'] }}" class="mt-2 w-full inline-flex items-center justify-center gap-1.5 h-9 rounded-lg bg-brand-blueLight text-brand-blue text-sm font-semibold hover:bg-brand-blue hover:text-white transition-colors">
            View options
          </a>
        @else
          <button type="button"
                  class="js-add-to-cart mt-2 w-full inline-flex items-center justify-center gap-1.5 h-9 rounded-lg bg-brand-blue text-white text-sm font-semibold hover:bg-brand-blueDark disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
