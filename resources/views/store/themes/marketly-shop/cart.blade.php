    ...
    <div x-data="miniCart()">
      <template x-if="!items.length">
        <div class="text-center py-20 bg-white border border-mkt-ink/15 rounded-lg">
          ...
          <a href="{{ route('store.shop') }}" class="mt-4 inline-flex h-11 px-6 bg-mkt-purple text-white font-bold text-xs eyebrow items-center rounded-md">{{ __('messages.ContinueShopping') ?? 'Start Shopping' }}</a>
        </div>
      </template>
