...
<nav class="lg:hidden fixed bottom-0 inset-x-0 z-30 bg-white/95 backdrop-blur border-t border-brand-line shadow-navUp pb-[env(safe-area-inset-bottom)]">
  <div class="grid grid-cols-5 h-16">
    <a href="{{ route('store.index') }}" class="flex flex-col items-center justify-center gap-0.5 {{ request()->routeIs('store.index') ? 'text-brand-teal' : 'text-slate-500' }}">
    ...
    <a href="{{ route('store.shop') }}" class="flex flex-col items-center justify-center gap-0.5 {{ request()->routeIs('store.shop') ? 'text-brand-teal' : 'text-slate-500' }}">
    ...
    <a href="{{ route('store.shop', ['sort' => 'price_asc']) }}" class="flex flex-col items-center justify-center gap-0.5 text-slate-500">
      <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3v18h18"/><path d="m7 14 4-4 3 3 5-6"/></svg>
      <span class="text-[10px] font-medium">Best Value</span>
    </a>
    <a href="{{ route('store.cart') }}" class="relative flex flex-col items-center justify-center gap-0.5 {{ request()->routeIs('store.cart') ? 'text-brand-teal' : 'text-slate-500' }}">
      ...
      <span class="cart-count absolute top-1 right-6 min-w-[16px] h-4 px-1 rounded-full bg-brand-amber text-white text-[10px] font-bold inline-flex items-center justify-center">0</span>
