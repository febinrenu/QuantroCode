...
@php
  ...
  // HomeLuxe's catalog has one real category (Home & Furniture) and no
  // subcategories, so every decorative room tile below links to that same
  // real, correctly-filtered category rather than the unfiltered shop page.
  $hlCategoryId = optional(($categories ?? collect())->first())->id;
  $hlCategoryUrl = fn () => route('store.shop', array_filter(['category' => $hlCategoryId]));
...
<main>
  ...
  <section class="max-w-[1440px] mx-auto px-5 mt-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
    @foreach($categoryTiles as $tile)
      <a href="{{ $hlCategoryUrl() }}" class="group">
  ...
  <section class="max-w-[1440px] mx-auto px-5 mt-10 mb-12">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
      @foreach($rooms as $room)
        <a href="{{ $hlCategoryUrl() }}" class="group text-center">
