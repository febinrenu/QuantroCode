...
<article class="product-card group bg-white border border-lj-ink/10 rounded-2xl overflow-hidden flex flex-col hover:shadow-cardHover transition-shadow">
  <a href="{{ $product['url'] }}" class="relative block aspect-square overflow-hidden bg-lj-cream" style="{{ !$product['image_url'] ? 'background:'.$product['placeholder_color'].'22' : '' }}">
    ...
    <button type="button" class="absolute top-2.5 right-2.5 w-8 h-8 rounded-full bg-white/90 shadow-card inline-flex items-center justify-center text-lj-inkSoft hover:text-lj-pink">
  ...
  <div class="product-body p-3 flex flex-col flex-1">
    <a href="{{ $product['url'] }}" class="product-title text-sm font-medium text-lj-ink line-clamp-2 hover:text-lj-purple" title="{{ $product['name'] }}">
      {{ $product['name'] }}
    </a>

    @if(!$product['hide_prices'])
      <div class="flex items-baseline gap-1.5 mt-1">
        <span class="price text-sm font-bold text-lj-ink">{{ $product['final_price_formatted'] }}</span>
        @if($product['compare_at_price_formatted'])
          <span class="text-xs text-lj-inkSoft line-through">{{ $product['compare_at_price_formatted'] }}</span>
        @endif
      </div>
    @else
    ...
    <div class="flex items-center gap-1 mt-1.5">
      ...
      <span class="text-[11px] text-lj-inkSoft">({{ (($product['id'] ?? 1) * 17) % 140 + 30 }})</span>
    </div>

    <div class="mt-auto pt-2">
      @if(!$product['hide_prices'] && count($product['variants']) > 0)
        <a href="{{ $product['url'] }}" class="w-full inline-flex items-center justify-center h-9 border border-lj-purple text-lj-purple text-xs font-bold rounded-full hover:bg-lj-purple hover:text-white transition-colors">
          {{ 'View options' }}
        </a>
      @elseif(!$product['hide_prices'])
        <button type="button"
                class="js-add-to-cart w-full inline-flex items-center justify-center gap-1.5 h-9 bg-lj-purple text-white text-xs font-bold rounded-full hover:bg-lj-purpleDeep disabled:opacity-40 disabled:cursor-not-allowed transition"
      ...
      @endif
      <div class="js-add-status text-[11px] text-lj-inkSoft min-h-[1rem] mt-1"></div>
    </div>
  </div>
</article>
