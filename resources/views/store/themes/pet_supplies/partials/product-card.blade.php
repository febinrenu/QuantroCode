...
<article class="pet-card product-card">
  <div class="pet-card-top">
    @if($p->category)
      <span class="pet-species-tag">{{ $p->category->name }}</span>
    @elseif($brandName)
      <span class="pet-species-tag">{{ $brandName }}</span>
    @endif

    <span class="pet-paw-tag">🐾 Pet Choice</span>
  </div>

  <div class="pet-media">
    <a href="#" class="pet-img-link js-quick-view"
       data-id="{{ $p->id }}"
       ...
       @click.prevent>
      <img src="{{ $imgUrl }}" alt="{{ $p->name }}" loading="lazy" class="pet-img">
    </a>

    @if($isPreorderActive)
      <span class="pet-badge-pre">{{ __('messages.PreOrder') }}</span>
    @elseif(!$isAvailable)
      <span class="pet-badge-out">{{ __('messages.OutOfStock') }}</span>
    @endif

    <button type="button" class="pet-inspect-btn js-quick-view"
            title="Inspect Pet Product"
            data-id="{{ $p->id }}"
            data-slug="{{ $productSlug }}"
            data-name="{{ e($p->name) }}"
            data-price="{{ number_format($minPrice, 2, '.', '') }}"
  ...
  <div class="pet-body product-body">
    <h3 class="pet-title product-title" title="{{ $p->name }}">
      {{ $p->name }}
    </h3>

    @if($unitName)
      <div class="pet-unit-tag">Size / Pack: {{ $unitName }}</div>
    @endif

    @if($descShort)
      <p class="pet-summary">{{ \Illuminate\Support\Str::limit($descShort, 80) }}</p>
    @endif

    <div class="pet-price-row">
      <div class="pet-price-block">
        @if(empty($hidePrices))
          <span class="pet-price font-mono price">{{ $currency }}{{ number_format($minPrice, 2, '.', ',') }}</span>
        @else
          <a href="{{ route('store.login.show') }}" class="text-xs text-orange-600 font-bold underline">Login for price</a>
        @endif
      </div>

      @if(empty($hidePrices))
        <button type="button"
                class="pet-btn-add js-add-to-cart"
    ...
    <div class="js-add-status pet-add-status"></div>
  </div>
</article>
