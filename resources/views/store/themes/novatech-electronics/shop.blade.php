    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        <aside class="space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4">
                ...
                <ul class="space-y-1.5 text-xs">
                    ...
                    @foreach($nvtCategories as $slug => $name)
                        <li>
                            <a href="{{ route('store.shop', array_filter(['preview_theme' => request('preview_theme'), 'category' => $slug])) }}"
            ...
            <div class="rounded-2xl bg-gradient-to-br from-[#1E1B4B] to-[#4338CA] p-6 text-white text-center shadow-md space-y-3">
                ...
                <a href="{{ route('store.shop', array_filter(['preview_theme' => request('preview_theme')])) }}" class="inline-block w-full py-2 rounded-xl bg-white text-indigo-900 font-bold text-xs uppercase tracking-wider hover:bg-indigo-50 transition-colors shadow-sm">
                    Shop Bundles
                </a>
            </div>
        </aside>
        ...
        <main class="lg:col-span-3">
            @if($products->isEmpty())
                <div class="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm space-y-4">
                    ...
                    <a href="{{ route('store.shop', array_filter(['preview_theme' => request('preview_theme')])) }}" class="inline-flex items-center justify-center px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold transition-all shadow-md">
                        Clear All Filters
                    </a>
                </div>
            @else
